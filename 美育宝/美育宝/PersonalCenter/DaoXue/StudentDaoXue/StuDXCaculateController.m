...
#import "StuDXCaculateController.h"
...
@interface StuDXCaculateController ()<LrdSuperMenuDelegate,LrdSuperMenuDataSource,NSURLSessionDelegate,UITableViewDelegate,UITableViewDataSource>
{
    NSString *_userName;
...
@implementation StuDXCaculateController
- (instancetype)init
{
    self = [super init];
    if (self) {
...
- (void)viewDidLoad {
    ...
    _menu = [[LrdSuperMenu alloc] initWithOrigin:CGPointMake(0, 0) andHeight:40];
    _menu.delegate = self;
...
- (NSArray *)getClassifybuffer:(NSInteger)index
{
    ...
    NSString* materialUrl = [NSString stringWithFormat:@"http://192.168.3.254:8082/GetDataToApp.aspx?action=getbjkc&logincode=%@&bjh=%@",_userName, _classNumber];
