...
@property (nonatomic, strong) LYAddViewController *addCon;
...
//        self.addCon = [[LYAddViewController alloc] init];
//        btn = self.addCon.addDaoXueBtn;
//        [btn addTarget:self action:@selector(clickImageToSelected:) forControlEvents:UIControlEventTouchUpInside];
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.frame = CGRectMake(0, 0, kWindowWidth - 30, 250);
...
//点击弹框
- (void)clickImageToSelected:(UIButton *)tap
{
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"添加资源" message:nil preferredStyle:UIAlertControllerStyleActionSheet];
...
{
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"增加资源" message:nil preferredStyle:UIAlertControllerStyleActionSheet];
    UIAlertAction *photo = [UIAlertAction actionWithTitle:@"从相册选取" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [self selectImageFromAlbum];
    }];
    UIAlertAction *camera = [UIAlertAction actionWithTitle:@"拍照" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
