...
#import "ShowWorksController.h"
#import "ClassesWorksController.h"
...
@interface WorksTabBarController ()
{
...
@implementation WorksTabBarController
...
//    self.title = @"ddddd";
    ...
    backButton.image = [UIImage imageNamed:@"gobackBtn"];
...
- (void)setUpTabBarController
{
    ...
    titleArray = [[NSArray alloc]initWithObjects:@"个人作品",@"班级作品",@"优秀作品", nil];
    self.title = titleArray.firstObject;
    ShowWorksController *shows = [[ShowWorksController alloc] init];
    shows.title = titleArray.firstObject;
    ClassesWorksController *classes = [[ClassesWorksController alloc] init];
    classes.title = titleArray[1];
    ExcelentWorksController *excelent = [[ExcelentWorksController alloc] init];
    excelent.title = titleArray[2];
    
    self.viewControllers = @[shows, classes, excelent];
}
...
- (void)configerUI{
    ...
    float interValX = KWIDTH/3-20;
    
    //创建3个按钮
    for (int i = 0; i<3; i++) {
        
        //图片
        UIImageView *imageView = [[UIImageView alloc]initWithFrame:CGRectMake((KWIDTH/2-30)/3+(20+interValX)*i, 6, 20, 20)];
        ...
        titleLabel.frame = CGRectMake((KWIDTH/3)*i, CGRectGetMinY(imageView.frame)+25, KWIDTH/3, 15);
        ...
        tabBtn.frame = CGRectMake((KWIDTH/3)*i, 0, KWIDTH/3, 49);
