...
static const CGFloat kVideoControlTitleLabelFontSize = 21.0;
...
- (void)layoutSubviews
{
    ...
    self.closeButton.frame = CGRectMake(CGRectGetMinX(self.topBar.bounds), CGRectGetMaxY(self.topBar.bounds)/2 - CGRectGetMaxY(self.closeButton.bounds)/2, CGRectGetMaxX(self.closeButton.bounds), CGRectGetMaxY(self.closeButton.bounds));
    self.titleLabel.frame = CGRectMake(CGRectGetMaxX(self.closeButton.bounds), CGRectGetMaxY(self.topBar.bounds)/2 - CGRectGetMaxY(self.titleLabel.bounds)/2, CGRectGetMaxX(self.topBar.bounds) - CGRectGetMaxX(self.closeButton.bounds) - 100, CGRectGetMaxY(self.titleLabel.bounds));
    ...
    self.timeLabel.frame = CGRectMake(CGRectGetMidX(self.progressSlider.frame) - 45, CGRectGetMaxY(self.progressSlider.frame) - 10, CGRectGetMidX(self.progressSlider.frame), CGRectGetMaxY(self.timeLabel.bounds));
...
- (UIButton *)playButton
{
    if (!_playButton) {
        ...
        [_playButton setImage:[UIImage imageNamed:[self videoImageName:@"pause"]] forState:UIControlStateNormal];
...
- (UIButton *)pauseButton
{
    if (!_pauseButton) {
        ...
        [_pauseButton setImage:[UIImage imageNamed:[self videoImageName:@"play"]] forState:UIControlStateNormal];
...
- (UIButton *)fullScreenButton
{
    if (!_fullScreenButton) {
        ...
        [_fullScreenButton setImage:[UIImage imageNamed:[self videoImageName:@"fullscreen"]] forState:UIControlStateNormal];
...
- (UIButton *)shrinkScreenButton
{
    if (!_shrinkScreenButton) {
        ...
        [_shrinkScreenButton setImage:[UIImage imageNamed:[self videoImageName:@"shrinkscreen"]] forState:UIControlStateNormal];
...
- (UISlider *)progressSlider
{
    if (!_progressSlider) {
        ...
        [_progressSlider setThumbImage:[UIImage imageNamed:[self videoImageName:@"point"]] forState:UIControlStateNormal];
...
- (UIButton *)closeButton
{
    if (!_closeButton) {
        ...
        [_closeButton setImage:[UIImage imageNamed:@"gobackBtn.png"] forState:UIControlStateNormal];
        _closeButton.frame = CGRectMake(0, 0, kVideoControlBarHeight, kVideoControlBarHeight);
    }
    return _closeButton;
}
...
- (UILabel *)timeLabel
{
    if (!_timeLabel) {
        ...
        _timeLabel.frame = CGRectMake(0, 0, kVideoControlTimeLabelFontSize, kVideoControlTimeLabelFontSize);
        _timeLabel.text = @"000/000";
    }
    return _timeLabel;
}
...
- (UILabel *)titleLabel
{
    if (!_titleLabel) {
        ...
        _titleLabel.frame = CGRectMake(0, 0, kVideoControlTitleLabelFontSize, 150);
//        _titleLabel.text = @"bad boy jfdksfh  njafkjnfjksakffnak fjakf kj";
    }
    return _titleLabel;
}
...
{
    if (name) {
        NSString *path = [NSString stringWithFormat:@"kr-video-player-%@",name];
        return path;
    }
    return nil;
}
